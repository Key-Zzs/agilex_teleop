...
- [ ] 臂体控制问题
  ...
  - [ ] ik问题（拷打松灵技术）
    - [ ] 控制频率？
    - [ ] 解算方法（可考虑采用其他 ik 库配合 [nero urdf 模型](https://github.com/agilexrobotics/agx_arm_urdf) 使用，推荐 [ikpy](https://github.com/ikpy/ikpy) 和 [frax](https://github.com/danielpmorton/frax)）
- [ ] 夹爪问题
  - [ ] [Pika 夹爪 sdk](https://github.com/agilexrobotics/pika_sdk) 适配
  - [ ] 响应速度（先移植 Pika 夹爪 sdk 到项目中，测试后问题依旧存在再尝试研究 sdk 和 [Server 端项目](nero/teleop/interface/nero_interface_server.py) 中 `left_gripper_goto()` 和 `right_gripper_goto()` 实现方式）
